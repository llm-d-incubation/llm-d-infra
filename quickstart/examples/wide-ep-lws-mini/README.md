...
1. Use the quickstart to deploy Gateway CRDS + Gateway provider + Infra chart (from `/llm-d-infra/quickstart`). This example only works out of the box with `Istio` as a provider, but with changes its possible to run this with `kgateway`.
   ...
   ./llmd-infra-installer.sh --namespace llm-d-wide-ep-mini -r infra-wide-ep-mini -f examples/wide-ep-lws/infra-wide-ep-mini/values.yaml --disable-metrics-collection
   ```

   **_NOTE:_** The release name `infra-wide-ep-mini` is important here, because it matches up with pre-built values files used in this example.
...
```bash
$ helm list -n llm-d-wide-ep-mini
NAME             NAMESPACE        REVISION    UPDATED                                 STATUS      CHART                        APP VERSION
infra-wide-ep-mini    llm-d-wide-ep-mini    1           2025-07-25 05:43:35.263697 -0700 PDT   deployed    llm-d-infra-v1.1.1           v0.2.0
ms-wide-ep-mini       llm-d-wide-ep-mini    1           2025-07-25 06:16:29.31419 -0700 PDT    deployed    llm-d-modelservice-v0.2.0    v0.2.0
```

1. You should all the pods you expect to (2 decodes, 1 prefill, 1 gateway pod, 1 EPP pod):

```bash
$ kubectl get pods -n llm-d-wide-ep-mini
NAME                                                   READY   STATUS    RESTARTS   AGE
infra-wide-ep-mini-inference-gateway-istio-7f4cf9f5-hpqg4   1/1     Running   0          55m
ms-wide-ep-mini-llm-d-modelservice-decode-0                 2/2     Running   0          22m
ms-wide-ep-mini-llm-d-modelservice-decode-0-1               2/2     Running   0          22m
ms-wide-ep-mini-llm-d-modelservice-epp-749696866d-n24tx     1/1     Running   0          22m
ms-wide-ep-mini-llm-d-modelservice-prefill-0                1/1     Running   0          22m
```
...
```bash
DECODE_PODS=$(kubectl get pods -n llm-d-wide-ep-mini -l "llm-d.ai/inferenceServing=true,llm-d.ai/role=decode" --no-headers | awk '{print}' | tail -n 2)
...
```bash
PREFILL_POD=$(kubectl get pods -n llm-d-wide-ep-mini -l "llm-d.ai/inferenceServing=true,llm-d.ai/role=prefill" | tail -n 1 | awk '{print}')
...
```bash
kubectl port-forward -n llm-d-wide-ep-mini service/infra-wide-ep-mini-inference-gateway-istio 8000:80
```
...
helm uninstall infra-wide-ep-mini -n llm-d-wide-ep-mini
```

## Customization

- **Change model**: Edit `ms-wide-ep-mini/values.yaml` and update the `modelArtifacts.uri` and `routing.modelName`
